...
describe("CreateMediaUseCase", () => {
    ...
    const createMediaUseCase = new CreateMediaUseCase(mediaRepo, cloudinaryServiceStub);
    let baseRequest = {
        type: "IMAGE",
    ...
    it("should return an error message when the file type is not supported", async () => {
        ...
        const unsupportedFileType = await createMediaUseCase.execute(request as CreateMediaDTO);
        expect(unsupportedFileType.isLeft()).toBe(true);
        expect(unsupportedFileType.value.isFailure).toBe(true);
        expect(unsupportedFileType.value.errorValue()).toBe("Unknown the media type 'AUDIO'");
    });
    ...
    it("should return an error message when the service has failed to persist the file", async () => {
        ...
        cloudinaryServiceStub.save = jest.fn().mockResolvedValueOnce(
           Result.fail("Internal error. Contact support.")
        );

        const createMediaUseCase = new CreateMediaUseCase(mediaRepo, cloudinaryServiceStub);
        const serviceFailedToPersist = await createMediaUseCase.execute(baseRequest as CreateMediaDTO);
        expect(serviceFailedToPersist.isLeft()).toBe(true);
        expect(serviceFailedToPersist.value.isFailure).toBe(true);
        expect(serviceFailedToPersist.value.errorValue()).toBe("Error encounter when saving file");
    });
    ...
    it("should return an error message when the all the required values are not provided", async() => {
        ...
        expect(serviceFailedToPersist.isLeft()).toBe(true);
        expect(serviceFailedToPersist.value.isFailure).toBe(true);
        expect(serviceFailedToPersist.value.errorValue()).toBe("Error encounter when saving file to provider");
    });
    ...
    it("should return an error message when the file informations failed to be persist in database", async() => {
        ...
        const createMediaUseCase = new CreateMediaUseCase(mediaRepoStub, cloudinaryServiceStub);
        const serviceFailedToPersist = await createMediaUseCase.execute(baseRequest as CreateMediaDTO);
        expect(serviceFailedToPersist.isLeft()).toBe(true);
        expect(serviceFailedToPersist.value.errorValue()?.message).toBe("An unexpected error occurred.");
    });
    ...
    it("should return nothing whe all fields are well provided and service works", async () => {
        ...
        expect(mediaRepoStub.save).toHaveBeenCalledWith(
            Media.create({
                ...
                description: "Here is the image",
            }, new UniqueEntityID(mediaId)).getValue()
        );
